...
External FFmpeg versions are supported on all plattforms (Windows, Linux, macOS) and can give access to newer video encoding codecs leading to smaller 
output files sizes and improved quality.

FFmpeg releases can be downloaded from the offical website `https://ffmpeg.org/download.html <https://ffmpeg.org/download.html>`__.
...
FFmpeg executable
  ...
  The outcome of the validation is displayed in the status field. If successful, the deduced path and ffmpeg version string will be displayed. 
  The executable needs to to be have its executable flag set (e.g. by running ``chmod +x ffmpeg`` on macOS/Linux) for it to be considered valid.
...
Codec
  ...
  Currently, `h264` and `h265` codecs are supported. The `h264` codec has widespread compatibility and performance. More modern `hvec` / `h265` codecs are 
  supported by more modern and can be used to further reduce file sizes at visually comparable quality.
...
Quality preset
  You can select a quality preset here. A higher quality preset will lead to larger files size, longer encoding time leading to an increased visual fideltiy of the 
  resulting video. 
  
  For external FFmpeg executables, they adjust the ``crf`` parameter of the ``ffmpeg`` command line interface. For 
  `h264` the ``crf`` value goes from ``26`` (quality: low) to ``19`` (quality: high). For `h265` the ``crf`` value goes from ``24`` (quality: low) to ``17`` (quality: high).
  The built-in FFmpeg encoder of OVITO does uses these presets to set FFmpeg's ``q_min`` and ``q_max`` parameters: ranging from ``3`` (quality: high) to ``8`` (quality: low).
  Note that previous version of OVITO defaulted to the "high" quality preset.
