...
This modifier produces a set of continuous lines by sampling the particle positions along the loaded simulation trajectory.
It can be used to visualize the trajectories of motion of particles in a static image.
...
For the latter, you first need to insert some selection modifier into the data pipeline to define the input particle selection.
The selection set is taken from the first frame of the trajectory.
...
OVITO automatically reads in all simulation frames one by one to samples the particle positions and produce the trajectory lines.
As part of this process, all upstream modifiers in the data pipeline are executed for each sampled trajectory frame.
The generated :ref:`scene_objects.lines` are stored in memory, and they will be updated automatically whenever you change
the upstream pipeline in some way.

.. image:: /images/modifiers/flow_trajectory_example.gif
  :width: 35%

This example animation shows a system of Lennard-Jones particles flowing around two obstacles.
The paths of motion of the red marker particles have been visualized using trajectory lines generated by the modifier.
The option :guilabel:`Show up to current time only` of the :ref:`Lines <visual_elements.lines>` visual element restricts
...
.. seealso:: :ref:`tutorials.marker_particles`

Sampling a particle property along the trajectories
"""""""""""""""""""""""""""""""""""""""""""""""""""

The modifier has an option to transfer a varying property of the particles onto the generated trajectory lines. In other words, the time-dependent particle property
...
the trajectory line were then visualized using the color mapping option of the :ref:`Lines <visual_elements.lines>` visual element:
...
Furthermore, note that the :ref:`Lines <visual_elements.lines>` visual element (see lower panel)
