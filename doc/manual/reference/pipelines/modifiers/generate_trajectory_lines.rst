...
The modifier automatically reads in all simulation frames one by one, sampling the particle positions to produce the trajectory lines.
As part of this process, all upstream modifiers in the data pipeline (if any) get computed for each sampled trajectory frame.
The generated :ref:`scene_objects.lines` are kept in memory, and they will be automatically regenerated whenever you change
the upstream pipeline in some way.

.. image:: /images/modifiers/flow_trajectory_example.gif
  :width: 35%

This demo animation shows a system of Lennard-Jones particles flowing around two obstacles.
...
of particles and/or timesteps can be prohibitively expensive in terms of required memory.
Thus, it typically is a good idea to restrict the generation of trajectory lines to a small group of particles of interest.
