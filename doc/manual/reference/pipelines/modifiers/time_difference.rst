.. _particles.modifiers.time_difference:

Time Difference |ovito-pro|
---------------------------

This modifier computes the difference between a selected particle property in the current frame and a reference frame. 
...
If the input property is named ``<property>``, the output is stored in a new property named ``<property> Difference``.  
The result has the same data type and number of components as the input.
...
By default, the modifier uses particle data from simulation timestep 0 as the reference frame.  
...
The *Time Difference* modifier assumes the number of particles remains constant over time.  
...
.. seealso::
    ...
    :py:class:`ovito.modifiers.TimeDifferenceModifier` (Python API)
