...
created by the :ref:`particles.modifiers.generate_trajectory_lines` modifier or the :py:attr:`DataCollection.lines.create() <ovito.data.DataCollection.lines>` method.
...
be gradually drawn while playing the animation, as in the examples shown on the right. This option may not be available for lines created
via the :py:attr:`DataCollection.lines.create() <ovito.data.DataCollection.lines>` method.
