...
:ref:`OSPRay <rendering.ospray_renderer>`, or VisRTX (!add link!).
...
instead go to :guilabel:`File` -> :guilabel:`Render on remote computer...`. This
opens the :guilabel:`Remote render settings` dialog window. 
...
OVITO Pro has generated a Slurm batch job script template, `submit.template`,
...
.. code-block:: bash
    ...
    # Run render
    cd $SLURM_SUBMIT_DIR
    srun -N ${SLURM_NNODES} -n ${SLURM_NNODES} --cpu_bind=none flux start python RenderTask.py

Once this file is set up we can rename and submit it to the queing system.

.. code-block:: bash

    daniel@magikarp:/scratch/daniel/render/remote_render_tutorial$ submit.template submit.sh
    daniel@magikarp:/scratch/daniel/render/remote_render_tutorial$ sbatch
    submit.sh
