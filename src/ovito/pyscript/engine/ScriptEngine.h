...
public:
    ...
    using TaskCreatedHandler = std::function<void(const TaskPtr&)>;
	...
	static int executeCommands(const QString& commands, RefTarget* contextObj, const TaskPtr& task, OutputHandler outputHandler, bool modifyGlobalNamespace, const QStringList& cmdLineArguments = QStringList());
	...
	/// \return The exit code returned by the Python script.
	/// \throw Exception on error.
	static int executeFile(const QString& file, RefTarget* contextObj, const TaskPtr& task, OutputHandler outputHandler, bool modifyGlobalNamespace, const QStringList& cmdLineArguments = QStringList());

	/// \brief Executes the given C++ function, which in turn may invoke Python code, in the context of this engine.
	/// \param contextObj An object (e.g. a DataSet) that serves as context of the script execution.
	/// \param task The asynchronous task object that serves as context of the script execution.
	/// \param stdoutSlot The optional name of the Qt slot of the context object that is called whenever the script produces console output.
	static int executeSync(RefTarget* contextObj, const TaskPtr& task, OutputHandler outputHandler, const std::function<void()>& func, bool importOvitoPackage = true);

	/// \brief Executes the given C++ function in the context of an object and a scripting engine.
	/// \param contextObj An object (e.g. a DataSet) that serves as context of the script execution.
	/// \param stdoutSlot The optional name of the Qt slot of the context object that is called whenever the script produces console output.
	static Future<void> executeAsync(RefTarget* context, OutputHandler outputHandler, const std::function<py::object()>& func, bool importOvitoPackage = true, TaskCreatedHandler taskCreatedHandler = {});
	...
	/// Requests cancellation of active script tasks whose context objects match the given predicate.
	static void cancelActiveScriptTasks(const std::function<bool(RefTarget*)>& predicate);
...
private:
	...
	struct ScriptExecutionContext {
		...
		ScriptExecutionContext(RefTarget* c, OutputHandler handler, TaskPtr t) : contextObj(c), outputHandler(std::move(handler)), task(std::move(t)) {
