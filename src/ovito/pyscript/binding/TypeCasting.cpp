...
bool type_caster<QVariant>::load(handle src, bool)
{
	...
	if(isinstance<dict>(src)) {
		QVariantMap map;
		dict pyMap = reinterpret_borrow<dict>(src);
		for(auto item : pyMap) {
			type_caster<QVariant> valueCaster;
			if(!valueCaster.load(item.second, false))
				return false;
			map.insert(castToQString(item.first), static_cast<QVariant&&>(valueCaster));
		}
		value = map;
		return true;
	}
	if(isinstance<sequence>(src) && !isinstance<str>(src) && !isinstance<bytes>(src)) {
		...
		for(size_t i = 0; i < seq.size(); ++i) {
			type_caster<QVariant> valueCaster;
			if(!valueCaster.load(seq[i], false))
				return false;
			listValue.push_back(static_cast<QVariant&&>(valueCaster));
		}
		value = listValue;
		return true;
	}
...
handle type_caster<QVariant>::cast(const QVariant& src, return_value_policy /* policy */, handle /* parent */)
{
	switch(static_cast<QMetaType::Type>(src.type())) {
		...
		{
			dict pyDict;
			const QVariantMap map = src.toMap();
			for(auto it = map.constBegin(); it != map.constEnd(); ++it)
				pyDict[pybind11::cast(it.key())] = pybind11::cast(it.value());
