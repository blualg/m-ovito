...
public:
	...
	ovito_class(py::handle scope, const char* docstring = nullptr, const char* pythonClassName = nullptr)
		...
		this->def(py::init([](py::args args, py::kwargs kwargs) {
			OORef<OvitoObjectClass> instance = static_object_cast<OvitoObjectClass>(OvitoObjectClass::OOClass().createInstance());
...
auto register_subobject_list_wrapper(PythonClass& parentClass, const char* wrapperClassName, ListGetterFunction&& listGetter)
{
	...
	using VectorType = std::decay_t<std::invoke_result_t<std::decay_t<ListGetterFunction>, ObjectType&>>;
	...
	return [](DelegatingModifierClass& mod, const QString& typeName) {
		...
		for(auto clazz : PluginManager::instance().metaclassMembers<typename DelegatingModifierClass::DelegateBaseType>(delegateType)) {
			if(clazz->pythonDataName() == delegateStr) {
				...
				OORef<typename DelegatingModifierClass::DelegateBaseType> delegate = static_object_cast<typename DelegatingModifierClass::DelegateBaseType>(clazz->createInstance());
		...
		throw Exception(QStringLiteral("'%1' is not a valid type of data element this modifier can operate on. Supported types are: (%2)")
