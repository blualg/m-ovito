...
Future<AsynchronousModifier::EnginePtr> CreateIsosurfaceModifier::createEngine(const ModifierEvaluationRequest& request, const PipelineFlowState& input)
{
	...
	DataOORef<SurfaceMesh> mesh = DataOORef<SurfaceMesh>::create(ObjectCreationParams::WithoutVisElement, tr("Isosurface"));
...
void CreateIsosurfaceModifier::ComputeIsosurfaceEngine::perform()
{
	...
	PropertyAccessAndRef<qlonglong> histogramData = DataTable::OOClass().createUserProperty(_histogram->elementCount(), PropertyObject::Int64, 1, tr("Count"), DataBuffer::InitializeMemory);
