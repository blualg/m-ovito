...
void FFmpegSettingsPage::insertSettingsDialogPage(QTabWidget* tabWidget)
{
    ...
    tabWidget->addTab(page, tr("FFmpeg")); // VIDTODO: Ich bin mit "FFmpeg" als Tabnamen nicht ganz glücklich. Wissen wirklich alle Nutzer sofort was FFmpeg ist und was damit gemeint ist? Vielleicht lieber "Video Encoding" oder so ?
    ...
    // VIDTODO: Text evtl. noch etwas verbessern. Er springt thematisch etwas zu stark hin und her, was das Verständnis erschwert (erst AVI/MOV, dann MKV/MP4, dann erneut AVI/MOV).
    // Vorschlag: "Note: The H.265 codec does not work with .avi and .mov file formats (fallback is H.264). Only .mkv and .mp4 file formats will use the H.265 codec."
    codecInfoLabel->setWordWrap(true);
    codecInfoLabel->setVisible(_ffmpegCodecName == "libx265"); // VIDTODO: Hier lieber konstant setVisible(false)? Ich frage mich, wieso hier nicht auch die Angabe eines externen FFmpeg als weiteres Kriterium mit einbezogen wird (so wie im folgenden Lambda).
    ...
    connect(_ffmpegCodec, &QComboBox::activated, this, [this, codecInfoLabel](int index) {
        ...
        const QString& path = _externalFFmpeg ? _ffmpegPath->text().trimmed() : ""; // VIDTODO: Nie "", immer QString(), um dynamische Konvertierung von C-String zu QString zur Laufzeit zu vermeiden. Der QString-Konstruktor kann ja nicht ahnen, dass er mit einem leeren C-String aufgerufen wird.
    ...
    _ffmpegQualityBox->setCurrentIndex(ffmpegQuality); // VIDTODO: Möglicher Bug wenn QMetaEnum die Enums nicht in Reihenfolge 0, 1, 2, ... liefert. Nutze QComboBox::findData()
...
void FFmpegSettingsPage::validateFfmpegPath()
{
    ...
    if(userInput.isEmpty()) {
        // VIDTODO: Die aktuelle Meldung ist evtl. nicht ausreichend, um dem Nutzer zu erklären, was zu tun ist. Vielleicht besser eine Aufforderung?:
        //
        // "Please provide the path to the FFmpeg executable on your computer or leave it empty to use the built-in FFmpeg encoder of OVITO, which does not support all codecs." Oder so ähnlich.
        // Evtl. könnte diese Meldung auch einen Hyperlink auf die FFmpeg homepage enthalten, so dass man direkter die Software herunterladen kann unter Windows.
        //
        // Ich wäre auch einverstanden damit, diese allgemeine Aufforderung dauerhaft anzuzuzeigen - oberhalb des Inputfeldes. Dann könnte die "Statusmeldung" hier kürzer und direkter ausfallen, so wie sie im Moment gewählt ist.
    ...
    if(!(path.endsWith("ffmpeg", Qt::CaseInsensitive) || path.endsWith("ffmpeg.exe", Qt::CaseInsensitive))) {
        ...
        _statusLabel->setStatus(PipelineStatus(
            ...
            tr("Provided path needs to end with 'ffmpeg' or 'ffmpeg.exe' to be valid, the built-in FFmpeg library will be used."))); // VIDTODO: Das sollten zwei getrennte Sätze sein. "Provided" kann man streichen.
        ffmpegPathValidated(false);
        return;
    }
    ...
    connect(
        ...
        this, // VIDTODO: Da hier "this" als Context-Objekt für die Lambda-Funktion verwendet wird, könnte es zu einem Problem kommen, wenn das finished Signal erst nach Schließen des Dialogs eintrifft. Dann würde der Handler nie ausgeführt und der QProcess leaken.
        [process, timeoutTimer, userInput, path, this](int exitCode, QProcess::ExitStatus exitStatus) {
            ...
            if(exitCode == 0 && output.contains("FFmpeg version", Qt::CaseInsensitive)) {
                ...
                // Im Moment ergeben sich zumindest unnötig lange Wiederholungen, wenn der Nutzer das ffmpeg executable über den file selection dialog auswählt:
                //
