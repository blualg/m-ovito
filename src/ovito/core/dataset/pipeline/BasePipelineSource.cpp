...
******************************************************************************/
void BasePipelineSource::postprocessDataCollection(PipelineEvaluationResult& result, const PipelineEvaluationRequest& request)
{
    ...
    if(!request.interactiveMode()) {
        registerActiveFuture(result);
    }

    result.postprocess(*this, [this, throwOnError = request.throwOnError(), interactiveMode = request.interactiveMode(), validityInterval = result.validityInterval()](SharedFuture<PipelineFlowState> future) -> PipelineFlowState {
        ...
        try {
            ...
            return PipelineFlowState(dataCollection(), PipelineStatus(ex, QStringLiteral(" ")), validityInterval);
        }
        catch(const std::bad_alloc&) {
            setStatus(PipelineStatus(PipelineStatus::Error, tr("Not enough memory.")));
            return PipelineFlowState(dataCollection(), status(), validityInterval);
        }
        catch(...) {
            if(throwOnError)
                throw;
