...
public:
    ...
    constexpr AffineTransformationT(const QMatrix4x4& m) noexcept
        : std::array<Vector_3<T>,4>{{
    ...
    template<typename U>
    constexpr auto toDataType() const noexcept -> std::conditional_t<!std::is_same_v<T,U>, AffineTransformationT<U>, const AffineTransformationT<T>&> {
    ...
    constexpr column_type& translation() { return column(3); }
    ...
    /// \brief Sets all elements of the matrix to zero.
    constexpr AffineTransformationT& operator=(Zero) {
        setZero();
        return *this;
    }
    ...
    /// \brief Sets the matrix to the identity matrix.
    constexpr AffineTransformationT& operator=(Identity) {
        setIdentity();
        return *this;
    }
    ...
    constexpr bool operator==(Zero) const {
    ...
    constexpr AffineTransformationT inverse() const {
    ...
    /// \sa determinant()
    constexpr bool inverse(AffineTransformationT& result, T epsilon = T(1e-16)) const {
        T det = determinant();
    ...
    inline constexpr const Matrix_3<T>& linear() const {
    ...
    static constexpr inline AffineTransformationT rotationX(T angle) {
        const T c = cos(angle);
        const T s = sin(angle);
    ...
    /// \param angle The rotation angle in radians.
    static constexpr inline AffineTransformationT rotationY(T angle) {
        const T c = cos(angle);
        const T s = sin(angle);
    ...
    /// \param angle The rotation angle in radians.
    static constexpr inline AffineTransformationT rotationZ(T angle) {
        const T c = cos(angle);
        const T s = sin(angle);
    ...
    static constexpr AffineTransformationT rotation(const RotationT<T>& rot);

    /// Generates a pure rotation matrix from a quaternion.
    static constexpr AffineTransformationT rotation(const QuaternionT<T>& q);

    /// Generates a pure translation matrix.
    static constexpr AffineTransformationT translation(const Vector_3<T>& t) {
    ...
    static constexpr AffineTransformationT scaling(T s) {
    ...
    static constexpr AffineTransformationT diagonal(T s1, T s2, T s3) {
        return AffineTransformationT(
                         s1, T(0), T(0), T(0),
                         T(0), s2, T(0), T(0),
                         T(0), T(0), s3, T(0));
    ...
    static constexpr AffineTransformationT scaling(const ScalingT<T>& scaling);

    /// Generates pure shear matrix (shear in the x-y plane).
    static constexpr AffineTransformationT shear(T gammaX, T gammaY) {
        return AffineTransformationT(
                         T(1), T(0), gammaX, T(0),
                         T(0), T(1), gammaY, T(0),
                         T(0), T(0), T(1),   T(0));
    ...
    static constexpr AffineTransformationT fromOpenGL(const T tm[16]) {
    ...
    static constexpr AffineTransformationT lookAt(const Point_3<T>& camera, const Point_3<T>& target, const Vector_3<T>& upVector) {
    ...
    ///                 around the view axis.
    /// \return A transformation from world space to view space.
    static constexpr AffineTransformationT lookAlong(const Point_3<T>& camera, const Vector_3<T>& direction, const Vector_3<T>& upVector) {
    ...
    /// \return \c true if the matrix is a pure rotation matrix; \c false otherwise.
    constexpr bool isTranslationMatrix() const {
...
/// \relates AffineTransformationT
template<typename T>
inline constexpr AffineTransformationT<T>& operator+=(AffineTransformationT<T>& a, const AffineTransformationT<T>& b)
{
...
template<typename T>
inline constexpr AffineTransformationT<T> AffineTransformationT<T>::rotation(const RotationT<T>& rot)
{
    T c = cos(rot.angle());
    T s = sin(rot.angle());
    T t = T(1) - c;
...
// Generates a pure scaling matrix.
template<typename T>
inline constexpr AffineTransformationT<T> AffineTransformationT<T>::scaling(const ScalingT<T>& scaling)
{
