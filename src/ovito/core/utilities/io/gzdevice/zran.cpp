...
/**
indexed_gzip: Copyright (C) 2016-2022 Paul McCarthy
...
/*
 * zran.cpp - indexed access to gzip files.
 *
 ...
 *
 * This source file has been adapted for OVITO.
 */
...
static int _zran_get_point_at(
    ...
    zran_point   **point       /* If an index point corresponding to the
...
static int _zran_get_point_with_expand(
    ...
    zran_point   **point       /* Place to store the identified point */
);
...
static int _zran_init_zlib_inflate(
    ...
    zran_point   *point         /* Pass in NULL to initialise for inflation
 ...
 * stored in fields of the zran_index   struct.
 *
...
static int _zran_inflate(
    zran_index   *index,            /* Pointer to the index. */
...
/* Initialise a zran_index   struct for use with the given GZIP file. */
int zran_init(zran_index   *index,
              void         *f,
              uint32_t      spacing,
              ...
              uint16_t      flags)
{
     ...
     */
    point_list = (zran_point*)calloc(1, sizeof(zran_point) * 8);
    if (point_list == NULL) {
        goto fail;
    }
...
uint64_t _zran_index_limit(zran_index   *index, uint8_t compressed) {
...
int _zran_expand_point_list(zran_index   *index) {
    zran_point   *new_list;
    uint32_t new_size = index->size * 2;

    zran_log("_zran_expand_point_list(%i -> %i)\n", index->size, new_size);

    new_list = (zran_point*)realloc(index->list, sizeof(zran_point) * new_size);
...
int _zran_free_unused(zran_index   *index) {
...
/* Deallocate memory used by a zran_index struct. */
void zran_free(zran_index *index) {
...
int _zran_invalidate_index(zran_index   *index, uint64_t from)
{
    uint64_t      i;
    zran_point   *p;
...
int zran_build_index(zran_index   *index, uint64_t from, uint64_t until)
{
 ...
 */
int _zran_get_point_with_expand(zran_index    *index,
                                uint64_t       offset,
                                uint8_t        compressed,
                                zran_point   **point)
{
...
int _zran_add_point(zran_index    *index,
                    uint8_t        bits,
                    uint64_t       cmp_offset,
    ...
    if (data == NULL) {
        ...
        point_data = (uint8_t*)calloc(1, index->window_size);
        if (point_data == NULL)
            goto fail;
    }
...
int _zran_init_zlib_inflate(zran_index   *index,
                            z_stream     *strm,
                            zran_point   *point) {
 ...
 * decompression.
 */
static int _zran_read_data_from_file(zran_index   *index,
                                     z_stream     *stream,
                                     uint64_t      cmp_offset,
                                     uint64_t      uncmp_offset,
 ...
 */
int _zran_find_next_stream(zran_index   *index,
                           z_stream     *stream,
                           int          *offset) {
...
static int _zran_validate_stream(zran_index   *index,
                                 z_stream     *stream,
                                 int          *offset) {
...
static int _zran_inflate(zran_index   *index,
                         z_stream     *strm,
                         uint64_t      offset,
                         uint16_t      flags,
     ...
     * zran_index  ->readbuf pointer.
     */
    if (inflate_init_readbuf(flags)) {
        index->readbuf = (uint8_t*)calloc(1, index->readbuf_size);
    ...
    if (inflate_init_stream(flags)) {
        ...
        if (start == NULL) {
             ...
             * the stream.
             */
            if (seekable_(index->f)) {
                if (fseek_(index->f, 0) != 0) {
                    goto fail;
                }
            }
 ...
 */
int _zran_expand_index(zran_index   *index, uint64_t until) {
 ...
 */
int zran_seek(zran_index    *index,
              int64_t        offset,
              uint8_t        whence,
              zran_point   **point)
{
...
uint64_t zran_tell(zran_index   *index) {
...
int64_t zran_read(zran_index   *index,
                  void         *buf,
                  uint64_t      len) {
     ...
     */
    discard = (uint8_t*)malloc(discard_size);
    if (discard == NULL) {
        goto fail;
    }
...
#if 0
/*
 * Store checkpoint information from index to file. File should be opened
 * in binary write mode.
 */
int zran_export_index(zran_index   *index,
                      void         *f) {
    ...
    /* Used for iterating over elements of zran_index  .list. */
    zran_point   *point;
    zran_point   *list_end;
    ...
    f_ret = fwrite_(ZRAN_INDEX_FILE_ID, sizeof(ZRAN_INDEX_FILE_ID), 1, f);
    if (ferror_(f)) goto fail;
    if (f_ret != 1)     goto fail;

    f_ret = fwrite_(&ZRAN_INDEX_FILE_VERSION, 1, 1, f);
    if (ferror_(f)) goto fail;
    if (f_ret != 1)     goto fail;

    /* Write flags (currently unused) */
    f_ret = fwrite_(&flags, 1, 1, f);
    if (ferror_(f)) goto fail;
    if (f_ret != 1)     goto fail;
 ...
 * binary read mode.
 */
int zran_import_index(zran_index   *index,
                      void         *f) {
     ...
     */
    new_list = (zran_point*)calloc(1, sizeof(zran_point) * std::max(npoints, (uint32_t)8));
    if (new_list == NULL)
        goto memory_error;
     ...
     */
    dataflags = (uint8_t*)calloc(npoints, 1);
    if (dataflags == NULL)
        goto memory_error;
    ...
    for (i = 0, point = new_list; i < npoints; i++, point++) {
         ...
         */
        point->data = (uint8_t*)calloc(1, window_size);
        if (point->data == NULL)
            goto memory_error;
        ...
        if (feof_(f, f_ret) && i < npoints - 1) goto eof;
        if (ferror_(f))                         goto read_error;
        if (f_ret != 1)                             goto read_error;
     ...
     */
    index->size    = std::max(npoints, (uint32_t)8);
