...
Future<PipelineFlowState> ParaViewVTMImporter::loadFrame(const LoadOperationRequest& request)
{
    ...
    Future<ExtendedLoadRequest> future = reduce_sequential(
        ...
        [](const ParaViewVTMBlockInfo& blockInfo, ExtendedLoadRequest& request) {
            ...
            if(!fileSource) throw Exception(QStringLiteral("Object requesting the data import has been deleted."));
                ...
                .then(*fileSource, [&request](const SharedFuture<FileHandle>& fileFuture) mutable -> Future<void> {
                    ...
                    try {
                        ...
                        for(const auto& filter : request.filters) {
                            Future<void> future = filter->loadDataset(request.blockInfo, file, request);
                            if(future.isValid()) return future;
                        }
                        ...
                        OORef<FileSourceImporter> importer =
                            dynamic_object_cast<FileSourceImporter>(FileImporter::autodetectFileFormat(file));
                        if(!importer) return Future<void>::createImmediateEmpty();
