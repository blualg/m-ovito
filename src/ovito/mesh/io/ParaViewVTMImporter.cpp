...
Future<PipelineFlowState> ParaViewVTMImporter::loadFrame(const LoadOperationRequest& request)
{
    ...
    Future<ExtendedLoadRequest> future = reduce_sequential(std::move(modifiedRequest), std::move(blockDatasets), ObjectExecutor(request.pipelineNode, true), [](const ParaViewVTMBlockInfo& blockInfo, ExtendedLoadRequest& request) {
        ...
        return Application::instance()->fileManager().fetchUrl(blockInfo.location).then(*fileSource, [&request](const SharedFuture<FileHandle>& fileFuture) mutable -> Future<void> {
