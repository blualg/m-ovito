...
void ParticleInspectionApplet::updateDistanceTable()
{
    ...
    ConstDataBufferAccess<Point3> posProperty = particles ? particles->getProperty(ParticlesObject::PositionProperty) : nullptr;
    ...
    if(_applet->_measuringModeAction->isChecked() && !renderer->isBoundingBoxPass()) {
        ...
        for(auto& element : _pickedElements) {
            ...
            if(const ParticlesObject* particles = flowState.getObject<ParticlesObject>()) {
                ...
                if(element.particleId >= 0) {
                    if(ConstDataBufferAccess<int64_t> identifierProperty = particles->getProperty(ParticlesObject::IdentifierProperty)) {
        ...
        DataBufferAccessAndRef<Point3G> lines = DataBufferPtr::create(n * (n - 1), DataBuffer::FloatGraphics, 3);
