...
bool GSDExporter::exportData(const PipelineFlowState& state, int frameNumber, const QString& filePath, MainThreadOperation& operation)
{
    ...
    if(ConstBufferAccess<int64_t> idProperty = particles->getProperty(ParticlesObject::IdentifierProperty)) {
    ...
    if(const PropertyObject* typeProperty = particles->getProperty(ParticlesObject::TypeProperty)) {
        ...
        // Build typeid array.
        ConstBufferAccess<int32_t> typeIdsArray(typeIds);
    ...
    if(ConstBufferAccess<FloatType> massProperty = particles->getProperty(ParticlesObject::MassProperty)) {
    ...
    if(ConstBufferAccess<FloatType> chargeProperty = particles->getProperty(ParticlesObject::ChargeProperty)) {
        // Apply particle index mapping and data type conversion:
    ...
    if(ConstBufferAccess<GraphicsFloatType> radiusProperty = particles->getProperty(ParticlesObject::RadiusProperty)) {
    ...
    if(ConstBufferAccess<QuaternionG> orientationProperty = particles->getProperty(ParticlesObject::OrientationProperty)) {
        // Apply particle index mapping and data type conversion.
    ...
    if(ConstBufferAccess<Vector3> velocityProperty = particles->getProperty(ParticlesObject::VelocityProperty)) {
        // Apply particle index mapping and data type conversion:
    ...
    if(const PropertyObject* angularMomentumProperty = particles->getProperty("angmom")) {
        if(angularMomentumProperty->dataType() == PropertyObject::FloatDefault && angularMomentumProperty->componentCount() == 4) {
            ConstBufferAccess<Quaternion> angularMomentumPropertyAccess(angularMomentumProperty);
            // Apply particle index mapping and data type conversion:
    ...
    if(const PropertyObject* bodyProperty = particles->getProperty("body")) {
        if(bodyProperty->dataType() == PropertyObject::Int32 && bodyProperty->componentCount() == 1) {
            ConstBufferAccess<int32_t> bodyPropertyAccess(bodyProperty);
    ...
    if(const BondsObject* bonds = particles->bonds()) {
        ...
        ConstBufferAccess<ParticleIndexPair> bondTopologyProperty = bonds->expectProperty(BondsObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = bonds->getProperty(BondsObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("bonds/typeid", typeIds->size(), 1, ConstBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const AnglesObject* angles = particles->angles()) {
        ...
        ConstBufferAccess<ParticleIndexTriplet> topologyProperty = angles->expectProperty(AnglesObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = angles->getProperty(AnglesObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("angles/typeid", typeIds->size(), 1, ConstBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const DihedralsObject* dihedrals = particles->dihedrals()) {
        ...
        ConstBufferAccess<ParticleIndexQuadruplet> topologyProperty = dihedrals->expectProperty(DihedralsObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = dihedrals->getProperty(DihedralsObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("dihedrals/typeid", typeIds->size(), 1, ConstBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const ImpropersObject* impropers = particles->impropers()) {
        ...
        ConstBufferAccess<ParticleIndexQuadruplet> topologyProperty = impropers->expectProperty(ImpropersObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = impropers->getProperty(ImpropersObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("impropers/typeid", typeIds->size(), 1, ConstBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
