...
bool GSDExporter::exportData(const PipelineFlowState& state, int frameNumber, const QString& filePath, MainThreadOperation& operation)
{
    ...
    if(ConstDataBufferAccess<int64_t> idProperty = particles->getProperty(ParticlesObject::IdentifierProperty)) {
    ...
    if(const PropertyObject* typeProperty = particles->getProperty(ParticlesObject::TypeProperty)) {
        ...
        // Build typeid array.
        ConstDataBufferAccess<int32_t> typeIdsArray(typeIds);
    ...
    if(ConstDataBufferAccess<FloatType> massProperty = particles->getProperty(ParticlesObject::MassProperty)) {
    ...
    if(ConstDataBufferAccess<FloatType> chargeProperty = particles->getProperty(ParticlesObject::ChargeProperty)) {
        // Apply particle index mapping and data type conversion:
    ...
    if(ConstDataBufferAccess<GraphicsFloatType> radiusProperty = particles->getProperty(ParticlesObject::RadiusProperty)) {
    ...
    if(ConstDataBufferAccess<QuaternionG> orientationProperty = particles->getProperty(ParticlesObject::OrientationProperty)) {
        // Apply particle index mapping and data type conversion.
    ...
    if(ConstDataBufferAccess<Vector3> velocityProperty = particles->getProperty(ParticlesObject::VelocityProperty)) {
        // Apply particle index mapping and data type conversion:
    ...
    if(const PropertyObject* angularMomentumProperty = particles->getProperty("angmom")) {
        if(angularMomentumProperty->dataType() == PropertyObject::FloatDefault && angularMomentumProperty->componentCount() == 4) {
            ConstDataBufferAccess<Quaternion> angularMomentumPropertyAccess(angularMomentumProperty);
            // Apply particle index mapping and data type conversion:
    ...
    if(const PropertyObject* bodyProperty = particles->getProperty("body")) {
        if(bodyProperty->dataType() == PropertyObject::Int32 && bodyProperty->componentCount() == 1) {
            ConstDataBufferAccess<int32_t> bodyPropertyAccess(bodyProperty);
    ...
    if(const BondsObject* bonds = particles->bonds()) {
        ...
        ConstDataBufferAccess<ParticleIndexPair> bondTopologyProperty = bonds->expectProperty(BondsObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = bonds->getProperty(BondsObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("bonds/typeid", typeIds->size(), 1, ConstDataBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const AnglesObject* angles = particles->angles()) {
        ...
        ConstDataBufferAccess<ParticleIndexTriplet> topologyProperty = angles->expectProperty(AnglesObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = angles->getProperty(AnglesObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("angles/typeid", typeIds->size(), 1, ConstDataBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const DihedralsObject* dihedrals = particles->dihedrals()) {
        ...
        ConstDataBufferAccess<ParticleIndexQuadruplet> topologyProperty = dihedrals->expectProperty(DihedralsObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = dihedrals->getProperty(DihedralsObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("dihedrals/typeid", typeIds->size(), 1, ConstDataBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
    ...
    if(const ImpropersObject* impropers = particles->impropers()) {
        ...
        ConstDataBufferAccess<ParticleIndexQuadruplet> topologyProperty = impropers->expectProperty(ImpropersObject::TopologyProperty);
        ...
        if(const PropertyObject* typeProperty = impropers->getProperty(ImpropersObject::TypeProperty)) {
            ...
            // Output typeid array.
            _gsdFile->writeChunk<uint32_t>("impropers/typeid", typeIds->size(), 1, ConstDataBufferAccess<int32_t>(typeIds).cbegin());
            if(operation.isCanceled()) return false;
        }
    }
