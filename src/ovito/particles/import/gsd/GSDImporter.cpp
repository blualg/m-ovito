...
void GSDImporter::FrameLoader::loadFile()
{
    ...
    if(PropertyAccess<GraphicsFloatType> radiusProperty = readOptionalProperty(gsd, "particles/diameter", frameNumber, ParticlesObject::RadiusProperty, particles(), &defaultDiameter, sizeof(defaultDiameter))) {
        ...
        for(GraphicsFloatType& r : radiusProperty)
            r /= 2;
    }
    if(PropertyAccess<QuaternionG> orientationProperty = readOptionalProperty(gsd, "particles/orientation", frameNumber, ParticlesObject::OrientationProperty, particles(), &identityQuaternion, sizeof(identityQuaternion))) {
        ...
        for(QuaternionG& q : orientationProperty)
...
PropertyObject* GSDImporter::FrameLoader::readOptionalProperty(GSDFile& gsd, const char* chunkName, uint64_t frameNumber, int propertyType, PropertyContainer* container, const void* defaultValue, size_t defaultValueSize)
{
    ...
    if(gsd.hasChunk(chunkName, frameNumber)) {
        ...
        if(prop->stride() == defaultValueSize) {
            DataBuffer::WriteAccess writeAccess(*prop);
            std::byte* dest = prop->buffer();
...
void GSDImporter::FrameLoader::parseEllipsoidShape(int typeId, QJsonObject definition)
{
    ...
    PropertyAccess<Vector3G> ashapeProperty = particles()->createProperty(ParticlesObject::AsphericalShapeProperty, DataBuffer::InitializeMemory);
...
void GSDImporter::FrameLoader::parseConvexPolyhedronShape(int typeId, QJsonObject definition, const QByteArray& shapeSpecString)
{
    ...
    SurfaceMeshAccess mesh(DataOORef<SurfaceMesh>::create(ObjectCreationParams::WithoutVisElement));
    ...
    if(roundingRadius > 0) {
        SurfaceMeshAccess roundedMesh(DataOORef<SurfaceMesh>::create(ObjectCreationParams::WithoutVisElement));
...
void GSDImporter::FrameLoader::parseSphereUnionShape(int typeId, QJsonObject definition, const QByteArray& shapeSpecString)
{
    ...
    DataOORef<TriMeshObject> sphereTemplate = DataOORef<TriMeshObject>::create(ObjectCreationParams::WithoutVisElement);
