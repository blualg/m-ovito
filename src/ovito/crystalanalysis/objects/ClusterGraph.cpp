...
IMPLEMENT_CREATABLE_OVITO_CLASS(ClusterGraph);
OVITO_CLASSINFO(ClusterGraph, "ClassNameAlias", "ClusterGraphObject");  // For backward compatibility with OVITO 3.10
OVITO_CLASSINFO(ClusterGraph, "DisplayName", "Clusters");

/******************************************************************************
* Constructor.
******************************************************************************/
void ClusterGraph::initializeObject(ObjectInitializationFlags flags)
{
...
******************************************************************************/
OORef<RefTarget> ClusterGraph::clone(bool deepCopy, CloneHelper& cloneHelper) const
{
    // Let the base class create an instance of this class.
    OORef<ClusterGraph> clone = static_object_cast<ClusterGraph>(DataObject::clone(deepCopy, cloneHelper));

    OVITO_ASSERT(clone->_clusters.empty());
    OVITO_ASSERT(clone->_clusterMap.empty());
    OVITO_ASSERT(clone->_clusterTransitions.empty());

    clone->_maximumClusterDistance = _maximumClusterDistance;
    clone->_clusters.reserve(clusters().size());
    clone->_clusterTransitions.reserve(clusterTransitions().size());
    for(const Cluster* cluster : clusters()) {
        Cluster* clonedCluster = clone->createCluster(cluster->structure, cluster->id);
        clonedCluster->atomCount = cluster->atomCount;
        clonedCluster->orientation = cluster->orientation;
        clonedCluster->symmetryTransformation = cluster->symmetryTransformation;
        clonedCluster->centerOfMass = cluster->centerOfMass;
        clonedCluster->color = cluster->color;
    }
    for(const ClusterTransition* transition : clusterTransitions()) {
        Cluster* clusterA = clone->findCluster(transition->cluster1->id);
        Cluster* clusterB = clone->findCluster(transition->cluster2->id);
        ClusterTransition* clonedTransition = clone->createClusterTransition(clusterA, clusterB, transition->tm, transition->distance);
        clonedTransition->area = transition->area;
    }
