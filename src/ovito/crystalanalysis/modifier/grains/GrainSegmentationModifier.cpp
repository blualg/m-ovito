...
void GrainSegmentationEngine1::applyResults(const ModifierEvaluationRequest& request, PipelineFlowState& state)
{
    ...
    if(_outputBondsToPipeline && modifier->outputBonds()) {
        ...
        PropertyAccessAndRef<FloatType> neighborDisorientationAngles = BondsObject::OOClass().createUserProperty(bonds.size(), DataBuffer::FloatDefault, 1, QStringLiteral("Disorientation"));
